...
/**
 * Created by Edge Tech Acadmey on 11/10/2016.
 */
...
//
//        Code in the try statement fails and throws an exception.
...
//
//        The statement that creates a FileWriter can fail for a number of reasons.
//              For example, the constructor for the FileWriter throws an IOException
//              if the program cannot create or write to the file indicated.
//
//        When FileWriter throws an IOException, the runtime system immediately stops executing the try block;
...
//              However, the FileWriter constructor doesn't have an appropriate exception handler,
//              so the runtime system checks the next method — the writeList method — in the method call stack.
...
//              The argument to the first exception handler is IndexOutOfBoundsException.
//              This does not match the type of exception thrown, so the runtime system checks the next exception handler — IOException.
//              This matches the type of exception that was thrown, so the runtime system ends its search for an appropriate exception handler.
...
//
//        In this scenario, all the statements within the scope of the try block execute successfully and throw no exceptions.
//              Execution falls off the end of the try block, and the runtime system passes control to the finally block.
//              Because everything was successful, the PrintWriter is open when control reaches the finally block,
...
//        The boldface code in the following sample shows the statements that get executed during this scenario.
    ...
    public static void main (String[] args) {
        ExceptionsTest exTest = new ExceptionsTest();
        exTest.writeList1();
        exTest.writeList2();
        exTest.writeList3();
    }
